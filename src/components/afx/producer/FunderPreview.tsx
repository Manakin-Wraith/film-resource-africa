...
import { deriveVisibility, VISIBILITY_META, meetsCorePackaging } from '@/lib/afx/constants';
...
/**
 * Read-only render of the producer's slate as a funder sees it on the
 * screening floor: bands (never raw figures), provenance badges, risk flags.
 * Rating/Deal Signal scores are staff- and funder-only and are deliberately
 * withheld here even though real funders do see them — this preview is about
 * how the slate reads, not about surfacing a score for the producer to chase.
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 16px', background: 'var(--afx-accent-soft)', border: '1px solid #D6D8F5', borderRadius: 10 }}>
        ...
        <span style={{ fontSize: 12.5, color: '#5E6066' }}>How your slate reads on the screening floor — figures appear as bands, every claim shows its provenance. Rating and Deal Signal scores are staff/funder-only and are not shown here.</span>
      </div>
      ...
      <div style={{ background: 'var(--afx-surface)', border: '1px solid #EAE8E3', borderRadius: 14, padding: '18px 22px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap' }}>
          <div style={{ flex: 1, minWidth: 180 }}>
            ...
            <div style={{ fontFamily: mono, fontSize: 11.5, color: '#9A9CA3', marginTop: 3 }}>{view.careerStage}</div>
          </div>
      ...
      <div style={{ background: 'var(--afx-surface)', border: '1px solid #EAE8E3', borderRadius: 14, padding: '18px 22px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {live.map((p) => {
            ...
            return (
              <div key={p.id} style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr 1fr', gap: 14, alignItems: 'center', padding: '14px 16px', border: '1px solid #F2F0EB', borderRadius: 10 }}>
                <div>
